// screens/MapScreen.js — Leaflet + CARTO (không cần API key)
...
export default function MapScreen({ navigation }) {
  ...
  // ── Xin quyền location + fetch markers ──────────
  useEffect(() => {
    const init = async () => {
  ...
  // ── Nút định vị — chỉ dùng expo-location (RN) ───
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── Thanh tìm kiếm (overlay trên map) ── */}
      ...
      {/* ── Nút định vị (chỉ React Native, không có trong WebView) ── */}
      <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  errorSub:    { fontSize: 13, color: '#9CA3AF', textAlign: 'center', paddingHorizontal: 32 },
  retryBtn: {
